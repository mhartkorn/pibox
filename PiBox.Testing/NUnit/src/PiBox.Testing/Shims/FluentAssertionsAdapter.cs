...
namespace FluentAssertions
{
    ...
    {
        public void ContainKey(string key) => Assert.That(Should, Does.ContainKey(key));
        public void ContainKey(string key, string message) => Assert.That(Should, Does.ContainKey(key), message);

        public void NotContainKey(string key) => Assert.That(Should, Does.Not.ContainKey(key));
        public void NotContainKey(string key, string message) => Assert.That(Should, Does.Not.ContainKey(key), message);

        public void ContainValue(string key) => Assert.That(Should, Does.ContainValue(key));
        public void ContainValue(string key, string message) => Assert.That(Should, Does.ContainValue(key), message);

        public void NotContainValue(string key) => Assert.That(Should, Does.Not.ContainValue(key));

        public void NotContainValue(string key, string message) =>
            Assert.That(Should, Does.Not.ContainValue(key), message);
    }
    ...
    public class TestableEnumerable<T>(IEnumerable<T> should) : TestableReferenceTypeBase<IEnumerable<T>>
    {
        ...
        public void BeEquivalentTo(IEnumerable<T> expectation) =>
            Assert.That(Should, Is.EquivalentTo(expectation));

        public void BeEquivalentTo(IEnumerable<T> expectation, string message) =>
            Assert.That(Should, Is.EquivalentTo(expectation), message);
    ...
    public class TestableReferenceType<T>(T should) : TestableReferenceTypeBase<T> where T : class
    {
        ...
        public void BeEquivalentTo(T expectation) => Assert.That(Should, Is.EqualTo(expectation));

        public void NotBeNullOrEmpty()
        {
            Assert.That(Should, Is.Not.Null);
            Assert.That(Should, Is.Not.Empty);
        }
    }
